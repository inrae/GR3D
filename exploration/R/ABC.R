...
# setwd("C:/WorkspaceJava/GR3D/exploration/R/")
## Read and test the argument
# for loading script from R:
#commandArgs <- function(bool) "ABC1"
workingDir = commandArgs(TRUE)
if (length(workingDir)==0) {
  stop("This script need a argument for the working directory")
}
outputDir = paste("exploration/R/",workingDir,"/simus/",sep="")
...
inputFile = paste(workingDir,"/ABC.yaml",sep="")
  ...
  function(parameters) {
    ...
    if (!file.exists(paste(outputFile,".csv",sep=""))) {
      ...
      arguments=c('-q', '-simDuration',simDuration, '-simBegin',simBegin,
        ...
        '-env',"data/input/BNtryRealBasins.xml",'-observers',"data/input/obsTryRealABC.xml", '-RNGStatusIndex', seed)
      library("rJava")
    ...
    # return the kappa
    read.csv(paste(outputFile,".csv",sep=""), skip=8, h=F, sep=";")[[2]]
  }
}
...
inputDir="data/input/"
assign("inputDir",inputDir)

model = salrun(
  jarfile = "GR3D-1.0-SNAPSHOT.jar",
  simDuration = 400,
  simBegin = 2)
#model(c(0,10,8,2,0.7))

library("EasyABC")
# rejection
# result=ABC_rejection(model=model, prior=prior, nb_simul=inputData$nbSimul, summary_stat_target=c(1.0), tol=0.9, use_seed=TRUE, progress_bar=TRUE)
# lenormand
result= ABC_sequential(method="Lenormand", model=model, prior=prior, nb_simul=inputData$nbSimul, summary_stat_target=c(1.0), p_acc_min=inputData$p_acc_min, use_seed=TRUE, progress_bar=TRUE)#, verbose=TRUE)
save(result,file=paste("exploration/R/",workingDir,"/result.data",sep=""))

# load(file='result.data')
png(file=paste("exploration/R/",workingDir,"/density.png",sep=""))
par(mfrow=c(2,2))
for (i in 1:4) {
