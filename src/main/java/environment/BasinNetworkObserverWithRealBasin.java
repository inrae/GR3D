...
public class BasinNetworkObserverWithRealBasin extends ObserverListener
	...
	private class DisplayComponent extends JComponent {
		...
		protected synchronized void paintComponent(Graphics g) {
			...
			// affine transmortaion to resize the drawing according to window
			// size
			// TODO fixed the same scalinf for x and y to avoid weird map
			// deformation
			...
			for (Basin basin : bn.getBasins()) {
				...
				// displayShape.getBounds2D().toString());
				shapeBasinMap.put(displayShape, basin); // to be used by the
														// mouseMoved()
